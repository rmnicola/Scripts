...
echo "Make sure to grab rclon-conf from your vault."
echo "https://vault.bitwarden.com/#/vault"
read
...
echo "Checking unzip tool."
set +e
for tool in ${unzip_tools_list[*]}; do
    trash=$(hash "$tool" 2>>errors)
    if [ "$?" -eq 0 ]; then
	echo "Found unzip tool $tool."
        unzip_tool="$tool"
        break
    fi
done  
...
echo "Version checked. Will install version $current_version."
